...
```
本人只将PatchTST_supervised中weather.sh修改和run_longExp.py部分参数改成不是必须输入的，在自己本地服务器测试跑通，验证了一下模型效果。
在项目中增加了项目代码中提到的部分数据。
如果有人看到这个项目中，只需要构建环境后，把weather.sh中cd后的路径换成自己run_longExp.py所在的绝对路径即可运行，或者直接运行run_longExp.py
项目作者源码：https://github.com/yuqinie98/PatchTST
